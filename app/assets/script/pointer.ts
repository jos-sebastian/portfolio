...
class Pointer {
  ...
  public updatePointerMoveData(
    ...
    this.texcoordX = posX / canvas.width;
    this.texcoordY = 1.0 - posY / canvas.height;
    this.delta.x = this.correctDeltaX(this.texcoordX - this.prevTexcoordX, canvas);
    this.delta.y = this.correctDeltaY(this.texcoordY - this.prevTexcoordY, canvas);
    if (hover) {
      this.moved = Math.abs(this.delta.x) > 0 || Math.abs(this.delta.y) > 0;
    } else {
